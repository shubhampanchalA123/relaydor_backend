...
const registerUser = async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      // B. Exists but NOT verified → resend OTP
      ...
      await sendSMS({
        to: mobile,
        message: `Your OTP for mobile verification is: ${mobileOTP}`,
      });

      return res.status(200).json({
        success: true,
        message: "Already registered but not verified. New OTP sent.",
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: "User registered. OTP sent to email & mobile.",
    });
  ...
  try {
    ...
    if (!mobile || !otp) {
      return res.status(400).json({
        ...
        missingFields: !mobile ? ["mobile"] : !otp ? ["otp"] : [],
      });
    }
    ...
    const otpRecord = await OTP.findOne({ mobile, otp, purpose: "mobile_verification" });
    if (!otpRecord) {
      return res.status(400).json({ success: false, message: "Invalid OTP" });
    }
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    if (email) {
      ...
      if (!user.verified) {  // New check added here
      ...
      if (!user.verified) {  // New check added here
        return res.status(403).json({ success: false, message: "only verifiend account can change there passsword" });
      }
    }
    ...
    if (email) {
      ...
      await OTP.findOneAndUpdate(
        { mobile, purpose: "forgot_password" },
        { otp, expireAt: expire },
        { upsert: true, new: true }
      );
